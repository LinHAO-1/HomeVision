...
for category, prompts in FEATURE_PROMPTS.items():
    for label, prompt in prompts.items():
        FEATURE_FLAT.append((label, prompt, category))

FEATURE_LABELS = [entry[0] for entry in FEATURE_FLAT]
FEATURE_PROMPT_TEXTS = [entry[1] for entry in FEATURE_FLAT]
FEATURE_CATEGORIES = [entry[2] for entry in FEATURE_FLAT]
...
try:
    ...
    if os.path.isfile(adapter_path) and os.path.isfile(adapter_meta_path):
        with open(adapter_meta_path) as meta_file:
    ...
    print(f"[adapter] Not loaded: {error}")
...
def compute_analysis(pil_img: Image.Image) -> tuple[dict, list[dict], list[dict], list[dict]]:
    ...
    with torch.no_grad():
        ...
        detected_room = ROOM_LABELS[best_room_idx] if top_room_score >= ROOM_THRESHOLD else "Unknown"
        room_type = {
            "label": detected_room,
        ...
        if adapter_model is not None:
            ...
            for i, probability in enumerate(probabilities):
                if probability >= adapter_threshold:
...
async def analyze_batch(files: list[UploadFile] = File(...)):
    ...
    for uploaded_file in files:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid image {uploaded_file.filename}: {error}")
        np_img = np.array(pil_img)

        room_type, amenities, features, adapter_predictions = compute_analysis(pil_img)
        ...
        if adapter_predictions:
            result["adapterPredictions"] = adapter_predictions
